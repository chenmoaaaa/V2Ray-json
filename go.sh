...
downloadV2Ray(){
    ...
    DOWNLOAD_LINK="http://192.168.179.128/v2ray-linux-${VDIS}.zip"
...
copyFile() {
    NAME=$1
    MANDATE=$2
    ERROR=`cp "/tmp/v2ray/v2ray-v3.0.1-linux-${VDIS}/${NAME}" "/usr/bin/v2ray/${NAME}"`
    if [[ $? -ne 0 ]]; then
...
installV2Ray(){
    ...
    if [[ ! -f "/etc/v2ray/config.json" ]]; then
      cp "/tmp/v2ray/v2ray-v3.0.1-linux-${VDIS}/vpoint_vmess_freedom.json" "/etc/v2ray/config.json"
      if [[ $? -ne 0 ]]; then
    ...
    if [[ -n "${SYSTEMCTL_CMD}" ]];then
        if [[ ! -f "/etc/systemd/system/v2ray.service" ]]; then
            if [[ ! -f "/lib/systemd/system/v2ray.service" ]]; then
                cp "/tmp/v2ray/v2ray-v3.0.1-linux-${VDIS}/systemd/v2ray.service" "/etc/systemd/system/"
        ...
        cp "/tmp/v2ray/v2ray-v3.0.1-linux-${VDIS}/systemv/v2ray" "/etc/init.d/v2ray"
...
main(){
    ...
    if [[ $LOCAL_INSTALL -eq 1 ]]; then
        ...
        getVersion
        if [[ $? == 0 ]] && [[ "$FORCE" != "1" ]]; then
            colorEcho ${GREEN} "Version ${NEW_VER} is already installed."
            exit
        else
    ...
    colorEcho ${GREEN} "V2Ray v3.0.1 is installed."
    rm -rf /tmp/v2ray
    return 0
}
